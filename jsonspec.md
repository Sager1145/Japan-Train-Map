...
| `company`        | string  |  否 | `""` | 面向乘客的车辆（运营）公司简称；直通车用 `/` 分隔多家公司（见 3.4）  |
...
| `company`    | `JR西日本`、`東京メトロ`、`台鐵`、`台灣高鐵`、`台北捷運`、`京急電鉄/都営地下鉄` |

**台湾公司名规范**：台湾行程的 canonical `company` 使用面向乘客的简称：
`台鐵`、`台灣高鐵`、`台北捷運`、`新北捷運`、`桃園捷運`、`台中捷運`、
`高雄捷運`、`阿里山林鐵`。导入旧存档时，已知的公司/机构全称会自动迁移
为这些简称；`route_policy.preferred_operator_names` 与
`route_sections[].operator_names` 仍保存官方数据集中的完整运营者名称，
以保证线路匹配精确。

**直通车约定**：跨公司直通运行的列车，在 `company` 中以 `/` 分隔写出**所有**参与公司（如 `京急電鉄/都営地下鉄`）。UI 检测到 `/` 即自动在显示处追加「直通」字样（清单、悬浮标签、popup），JSON 中**不需要**额外的直通字段。

**对轨道渲染（寻路）的影响 —— 软偏好**：`train_type` 与 `company` 共同决定路线倾向渲染在哪条轨道上，但均为**软偏置**、不会造成断线；显式的 `route_policy` 永远优先。日本使用 N02 事业者种别；台湾使用官方 TDX `LineName` / `OperatorName` 和预计算官方 shape：

1. 当 `route_policy.allowed_institution_type_codes` 为缺省全量 `["1","2","3","4","5"]` 时，求解器按 `train_type` + `company` 推导出更窄的事业者种别偏好：日本含 `新幹線` → `["1"]`；JR 公司（非新干线）→ `["2"]`；`都営`/公营 → `["3"]`；地下鉄/私铁 → `["4"]`；第三セクター → `["5"]`。台湾 `台灣高鐵` → `["1"]`、`台鐵` → `["2"]`、各地捷运与 `阿里山林鐵` → `["3"]`。推导出多个信号时取并集；无信号则保持全量。配合默认 `institution_filter_mode: "soft"`，非匹配轨道只是加罚、仍可借道。
2. `company` 中的每家公司（`/` 拆分后）映射为官方运营者名并并入 `preferred_operator_names` 软偏好；常用简称自动换算为正式名（`JR西日本`→`西日本旅客鉄道`、`東京メトロ`→`東京地下鉄`、`台鐵`→`國營臺灣鐵路股份有限公司`、`台北捷運`→`臺北大眾捷運股份有限公司` 等），未知名称原样使用。
...
为空。阿里山林铁的 19 个官方 `MARKID` 另以 canonical `AFR-*` 站号提供查询，当前
`byCode` 共 1136 条。当前 TDX/PTX 快照没有发布任何简体站名字段，故全部简体值按上述
回退规则生成。
