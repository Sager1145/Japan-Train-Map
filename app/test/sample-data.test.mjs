...
test("precomputed manifests contain no wall-clock build metadata", async (t) => {
  const directories = [
    "sample-data",
    "sample-data-tw",
    "sample-data-hk",
  ...
  for (const directory of directories) {
    try {
      const manifest = await readJson(
        path.join(APP_DIR, "data", directory, "manifest.json"),
      );
      assert.equal(
        Object.hasOwn(manifest, "generated_at"),
        false,
        `${directory}/manifest.json must be reproducible`,
      );
      checked += 1;
    } catch (error) {
  ...
  if (!checked) t.skip("precomputed manifests are generated by npm run build");
});
