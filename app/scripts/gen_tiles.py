...
if KIND == "rail":
    LINE_RGBA = (110, 110, 110, 150)
    LINE_W = 1 * SS
    DRAW_DOTS = False
else:
    FOOT_RGBA = (70, 70, 70, 90)
    FOOT_W = 2 * SS
    DOT_R = 3 * SS
    DOT_FILL = (255, 255, 255, 230)
    DOT_OUTLINE = (60, 60, 60, 230)
    DRAW_DOTS = True
        ...
        if (tx1 - tx0) <= 4 and (ty1 - ty0) <= 4:
            for tx in range(tx0, tx1 + 1):
                for ty in range(ty0, ty1 + 1):
                    buckets[(tx, ty)].append(line)
        else:
            for a, b in zip(line, line[1:]):
                for tx in range(
                    int(min(a[0], b[0])) // TILE, int(max(a[0], b[0])) // TILE + 1
                ):
                    for ty in range(
                        int(min(a[1], b[1])) // TILE, int(max(a[1], b[1])) // TILE + 1
                    ):
                        buckets[(tx, ty)].append([a, b])
        if DRAW_DOTS:
            dx, dy = g(*disp(ft), z)
            dots[(int(dx) // TILE, int(dy) // TILE)].append((dx, dy))
    NMOD = int(os.environ.get("NMOD", "1"))
    NREM = int(os.environ.get("NREM", "0"))
    ...
    for tx, ty in tiles:
        ...
        for line in buckets.get((tx, ty), []):
            ...
            if len(pts) >= 2:
                d.line(
                    pts,
                    fill=(LINE_RGBA if KIND == "rail" else FOOT_RGBA),
                    width=(LINE_W if KIND == "rail" else FOOT_W),
                    joint="curve",
                )
        if DRAW_DOTS:
        ...
        td = os.path.join(OUT, str(z), str(tx))
        os.makedirs(td, exist_ok=True)
        img.save(os.path.join(td, f"{ty}.png"))
        total += 1
