...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    specs: Tuple[LineSpec, ...] = (
        ...
        # ONE railway with a branch, not two. 中和新蘆線 is numbered O01
        # (南勢角) to O21 (迴龍); 蘆洲 hangs off 大橋頭 (O12) and its own
        # stations are numbered O50–O54. The 蘆洲 row therefore carries only
        # its own run, and BOTH rows carry the line's name — which is what
        # makes the renderer treat the 12 stations they share as a trunk and
        # its branch, drawn over the same coordinates, instead of as two
        # independent railways to be pulled into parallel lanes
        # (app/scripts/railway/lib/parallel-corridors.mjs corridorRenderMode).
        LineSpec(
            "tw-trtc-o-luzhou",
            "中和新蘆線",
            "Zhonghe-Xinlu Line",
