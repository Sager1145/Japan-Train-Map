...
**R11b 一站一点：兄弟笔画共用的岔口站必须是同一个坐标。** 全境 44 个这样的站本来
就共用坐标，7 个不共用的把车站画成两个点、中间一个洞（札幌 相距 208 m）。
共用点必须从 canonical／主笔画已有的真实站台锚点中选出，不能取两条轨道之间的平均点；
没有 unsuffixed 主笔画时，由最长实体笔画选点。其他 sibling stroke 的相邻区间端点焊到该节点，
paired alignment 则只共享 `railwayIdentity`、不参加坐标焊接。
**配对走向豁免**：湯檜曽 两个站台相距 73 m 正是要画的事实，不是要抹平的分歧。
...
**R13 大站的点位按「本线自己的站台」落，站台按线路匹配。** 東京 是范式：每条线的
点必须落在**它自己的列车所停的站台群**上，且仍在它自己画出的轨道上——東海道新幹線
在最东侧岛（14-19），東北新幹線 在自己的 20-23 群（西侧一岛），上野東京ライン（東北線）
与 東海道線 同用地面中间要素 → **一个共享 junction point**（先选中间站台，再焊到该站台
旁南北实体轨道的共同 OSM 节点；不是两个恰好同坐标的独立站点），
総武線 在総武地下ホーム，且从同一点沿 N02 归在東海道線下的地下轨道向南接新橋—品川—
西大井；该笔画按 Apple 地图以総武線的蓝色显示。京葉線 在京葉地下
ホーム。一组多站台要素时**选顺着该线走向的那一个**，不选文件序
第一个；同一 N02 线路的主线与支线使用不同层站台时，按笔画局部选择，不得用一个全局点把
地下支线拖回地面。
N02 把 東北新幹線 的站区轨道与站台要素逐顶点抄成 東海道新幹線 的（两新干线一点重合）——
这类源级复制缺陷用**登记制几何补丁**修（evidence/tokyo-station-platforms.json，OSM 实测
中线，逐顶点匹配失配即停机），与 paired-alignment-geometry.json 同一先例。点位选定以后，
东京相邻区间也必须沿底图同源的 OSM 实体轨道登记：東海道新幹線 東京—品川、東北新幹線
東京—上野、共用地面线 東京—神田／有楽町、総武地下线 東京—新日本橋／新橋；不能再以
N02 的稀疏站间直线连接正确点位。登记几何可按审计站序自动反转。東北線—東海道線还必须
像札幌的函館線两笔画一样共享 railway identity 与 lane：北段 → 同一实体节点 → 南段的切线
转角应小于 5°，不能因轨道吸附或屏幕车道偏移而拆成两个点。

**R13.1（2026-08-21 大宮）一条 N02 线名在同一站有两组站台时，必须拆成两条包内线路。**
compact-v1 每条线每站只有一行，所以一条线在一个站只能有一个点。N02 把 埼京線 归在 東北線
名下，包因此把 北与野…北赤羽 焊在 東北本線 站序的最前面：支线**根本没有 大宮**（N02 明明
测了西侧站台和一条 27 顶点区间），而 大宮 那一行锚在宇都宮線的东侧岛上。结果是乘车切片
被投影到唯一带 大宮 的 東北線 笔画（东侧干线）上，再由 `snapEndpoint` 拉一条 98 m 直弦
横穿车站——用户看到的就是那个 73° 直角拐弯。
修法不是放宽弦长，而是按 `split-interleaved-branches.mjs` 的先例把支线拆出来
（`split-saikyo-branch.mjs` → `東北線-5`），并且**junction 站带着自己的锚过去**：埼京線 与
川越線 同用 大宮 19・20番線（西侧），所以支线的 大宮 落在 川越線 已经在用的那个坐标上，
两条直通线在同一个点相遇；宇都宮線・高崎線・京浜東北線 保留东侧的点。一站两点在这里
**是要画出来的事实**，`paired-alignments.test.mjs` 的「一站一点」因此对 `002914` 例外，
理由与 湯檜曽 同。

**R13.2 提示行不能让一条线到达它不停靠的站台。** `canonicalizeRouteFeature` 先用
`route_sections` 的 line_names 挑候选；候选笔画离本跳任一站台超过 60 m 时改在全网重挑，
并且只有新笔画两端都 ≤25 m 才替换（品鶴線 自 東京 改造后归在 総武線-3 名下，
湘南新宿ライン 的 新川崎—鶴見 因此曾被画到相鉄直通線上、偏 106 m）。介于两者之间的分歧
是「哪个站台」之争，提示行仍然更可信，交给
`validate-route-station-approach.mjs` 报告而不是粉饰。

**R13.3（2026-08-21 岸里玉出）站台归属只能问站台，不能问「OSM 把这段轨道叫什么」。**
OSM 的轨道名是**法定归属**：一条线开头的几公里跑在别人的线路名下时，它自己的站台离
「叫它名字的 way」可以有几百米。岸里玉出 就是范式——高野線 从这里起算里程，天下茶屋—
岸里玉出 那两股是 南海本線 的资产，OSM 也就照 南海本線 命名；于是审计量出「高野線 站台
离 高野線 way 185 m，南海本線 岛式站台离 way 1.2 m」，把 高野線 的点判给了 南海本線 的
3・4番線，`platform_assignments` 据此写下一行，包里 岸里玉出→帝塚山 只好用一条 211 m 直弦
横穿车站回到自己的轨道。**正确的判据有两个**：量 OSM 的 `public_transport=platform` 面
（它自带 `ref` 番線，岸里玉出 的 ref=1/2 是 高野線、ref=3;4 是 南海本線、ref=6 是 汐見橋線），
以及量站台要素**自身的走向轴**（高野線 152.5° 对 南海本線 12.9°，差 40°，写进
`line_axis_bearing_degrees` 后源数据一动就停机）。运营商自己的のりば表是第三重佐证。
一条 N02 线名的两个笔画各停一组站台时，用 `display_part_platforms` 按笔画站序给分线自己的
选择（汐見橋線 留在西侧岛，高野線 主线上东侧 1・2番線；**大阪 同型**——`東海道線-2` 就是
梅田貨物線，2023-03-18 うめきた 开业后停 21–24番のりば 的地下站台，继承干线的地面岛式站台就得
拉 180 m 弦横穿站场），落包一律走 `scripts/railway/apply-registered-platform-picks.mjs`
（读账本、幂等、只换那一个顶点、km 按边长差重述；`structure` **只在行尾越过新线长时**才按
替换边线性重映射）。`paired-alignments` 的「一站一点」随之对
`jp-南海電気鉄道-高野線:007517`、`jp-西日本旅客鉄道-東海道線:007068` 例外——1993 年拆掉 東・西連絡線 之后这两组站台之间
根本没有轨道，画成一个点就是画了一个不存在的接轨。求解侧要改**两处**，都不在渲染端：
其一是数据——N02 把 高野線 的站台同时挂在 南海本線 名下，`resolveStationForTrain` 的「离本车
其他停站最近」在同一车站的两个站台之间没有意义，删掉那一行错挂
（`migrations/fix-kishinosato-tamade-mainline-platform-2026-08-21.py`）。其二是
`resolveRouteEndpointStationCandidates`：它按「同名 + 同机构 + 1800 m 内」补候选，好让
站码写错线的跳（大分 挂 久大線 而 ソニック 走 日豊線）仍能推出真线路；**但补进来的是别的站台**，
route_section 已经写明 `line_names` 时只允许补**属于该线**的那些。岸里玉出 由此只剩 3・4番線
（东侧那条路径短 60 m，Dijkstra 本来必选东侧），而「站码写错线」的用途分毫不损——匹配 section
的那个候选照补。

**R13.4（2026-08-21）「区间首尾 300 m 有没有轨道」是一条独立的验收，要有自己的审计。**
`validate-station-approach-chords.mjs`：量每条区间**首尾各 300 m**到最近 OSM 活线的距离，
以区间本体中位数为环境值（>15 m 让给 basemap 审计），≥25 m WARNING、≥45 m ERROR；
**同时索引 disused/abandoned/razed**，压在废弃或停运线位上的一律不报（津軽線 蟹田—三厩）。
它填的是三个既有审计都看不见的缝：anchoring 只问「圆点在不在线上」（弦正好落在圆点上），
basemap-alignment 要 ≥150 m 的连续 >50 m 段（211 m 弦峰值 84 m 也够不上），
route-station-approach 问的是乘车线而不是全网线。首轮全量：jp 8580 区间 10 ERROR，
**同类（该站该线有多个 N02 站台要素可选）只有 大阪 一条**，已修；其余 9 条不是选站台问题
（7 条该站该线只有一个要素；姫路 是 anchor override 把点搬上高架岛而 N02 轨道仍在旧地面线位；
長町 是复线走廊单中心线偏 40–47 m），要动它们得用 OSM 几何补丁，属另一个决定。
tw 2（台北捷運 小南門 地下、阿里山線 屏遮那）、hk/mo 0、kr 2 个 4–10 m 噪声。
