...
## 6.13 上下行分线：几何是推导的，方向是有来源的（2026-08-16）

N02 表达「两个方向走不同轨道」的方式是**在分离段内的每个车站多存一条站台记录**。
因此判据是**连续多站台段**，而不是逐区间判——且该段**结束于下一个普通车站**，
那里两方向共用站台，即会合点。上越線：水上(1)、湯檜曽(2)、土合(2)、土樽(1)
→ 分离段 湯檜曽→土合→土樽，14.48 km，在 土樽 接回本线。
按区间判会得到停在山肚子里的 4.13 km 断头笔画。

**闸门契约**：改判整段后候选 3→42，绝大多数不是分线（近鉄 大阪上本町 的第二站台是
地下 難波線 层；新橋 的是 汐留 货物线，1.2 km 间距画 8.9 km）。几何**无法**区分，
按距离排序 汐留 甚至排第一。所以：

- **几何推导**（站台段 + 轨道图最短路），
- **方向有来源**（`data/raw/railway/jp/evidence/paired-alignment-directions.json`），
- **两者都满足才画**，被拒候选逐条进构建 note，不静默丢弃。

**第二类判据**（站台看不见的分线）：两方向在每个车站会合、只有站间轨道分开时，
两端站台都只有一条。做法是**切掉本区间用掉的轨道，再问图有没有另一条不重用它的路**。
`path_between` 的 `exclude` 参数本来就是为此存在的。上下界 150 m–2 km：
低于是同一条轨道存了两遍；高于是另一条路线（函館線 答 13 km 外的 砂原線）。

**全网普查已归档**：中央本線 3 处、羽越線 5 处、奥羽線 2 处现实存在但**本数据集画不了**，
理由逐条写在证据文件的 `researched_not_drawn`。最值得记的是 羽越線 村上–間島：
N02 **确实**存了第二条对齐（偏 778 m，与 OSM 770 m 吻合），但拆成两截、
中间断开 **3.4 km**——焊接即造轨道，故不画。

**验证判据**（可重复）：问「有多少 N02 轨道离已画几何超过 150 m」。
土合 一带答 **0 m** —— 这是上り線已完整绘出的独立证明。

### 6.13.1 分线必须两端都接上（2026-08-16）

上面那版只closed了**远端**（会合站 土樽）。近端从自己的站台起画就停住，
于是 湯檜曽 处上り笔画离下り线 **71 m 悬空**——地图上就是一条挂在半空的支线。

**两条bore不在车站分开。** OSM 的 新清水トンネル（way/24399161）洞口在
36.80172,138.98580，即 湯檜曽 上り站台**以南 117 m**。而 compact-v1 没有
「首站之前的轨道」这个位置：`decodeIntervals()` 会把每个区间端点钉到站点行上
（`rail-network.js:734`），所以任何前置几何都会被渲染器抹掉。

**做法**：分离段从**两方向仍共用的最后一站**起算（上越線 = 水上），
到会合站止。水上→湯檜曽 两笔重合，之后在洞口分开。与远端完全对称。
代价是 3.5 km 重合绘制；这是双线区间的事实，且同一条铁路的重合笔画不占车道。

**为什么必须用 OSM 而不能只看 N02**：N02 只说「站台不同」，不说轨道怎么走。
拉了该段全部 上越線 way 后，方向归属可独立复核：
...
逐区间对全部 OSM 轨道：水上–湯檜曽 10 m / 湯檜曽–土合 11 m / 土合–土樽 17 m（中位）。
两个站台各自落在自己那条 bore 上（13 m、11 m）。OSM 还直接给出 bore 名——
清水トンネル 9,698 m（上り）、新清水トンネル 13,510 m（下り）——与文献 9,702 / 13,490 吻合。

**分离幅度闸门**：只按「连续多站台段」判会在全境找出 43 条，绝大多数是普通复线——
中央本線 笹子/新笹子 两洞相距 **25 m**，路面电车对向站台相距几米。
故要求两线最宽处 **≥150 m**：上越線 840 m、北陸線 鳩原ループ 通过，其余落选。

**回归测试**：`test/paired-alignments.test.mjs` 现在钉「配对笔画首尾两点都必须落在
本线几何 1 m 内」。旧套件只查两 bore 互不相同、且远端接回，故近端悬空能通过。

### 6.13.2 第二条 bore 的几何取自 OSM（2026-08-17）

接上了以后仍**不贴合轨道**：N02-25 对第二条股道只存**一条粗中心线**，
湯檜曽ループ 处离 OSM 与底图共同画出的轨道最远 **222 m**。

**判据**：OSM 把两条 bore 按名字分开存（第一〜第四湯桧曽トンネル＋清水トンネル
对 新清水トンネル），故可逐 way 取出。存成
`data/raw/railway/jp/evidence/paired-alignment-geometry.json`（带 way id，
离线可复现），由 `load_pair_geometry()` 读入，在 `split_alternate_segments()`
按区间替换，两端仍钉回本包站台锚点。
...
| 坑 | 症状 | 判据 |
| --- | --- | --- |
| 直接跑最短路 | 最短路**结构上就是下り线**：松川ループ 整段画到下り，湯檜曽 螺旋被抄近道剪成 120° 弯 | 按隧道名封掉下り bore（新清水／新松川）再路由 |
| 全区间都换 OSM | 水上–湯檜曽 引入段两方向共用一条实测中心线，换源即产生**假平行轨** | 只替换构建器已判定分离的区间；引入段保留 N02 以逐点重合 |

**明确不采用 OSM 的区间**：土樽–越後中里（松川ループ）。OSM 有 松川トンネル
（way/530408533）但**两端都没接到地面轨道**，上り bore 是孤立碎片；
最短路会返回 新松川トンネル（下り）。该段保留 N02，与 OSM 相差 ≤105 m 且螺旋正确。

```text
湯檜曽–土合   离真实轨道 221 m -> 6 m        土合–土樽  60 m -> 7 m
```

**顺带修掉一个测试缺陷**：螺旋判据原本累加**整段净转角**，会互相抵消——
上り爬 湯檜曽 螺旋转一向、再经 清水トンネル 马蹄弯转回来，净值 −120°，
把明明存在的螺旋藏掉。改量**沿线任意一段的最大连续转角**（累计和的峰谷差）：

```text
上り 水上–土樽    405°   下り 143°
上り 土樽–越後中里 367°   下り 200°
```
